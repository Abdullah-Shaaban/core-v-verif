...
void conv2d_3x3(int8_t *o, int8_t *i, int8_t *f, int32_t num_rows, int32_t num_columns) {
  ...
  for (int32_t c = 0; c < num_columns; c += block_size_c) {
    ...
    for (int32_t r = 0; r < num_rows; r += 1) {
      ...
      // Multiply the filter column by the whole image rows, and accumulate
      asm volatile("vmacc.vx v0, %0, v8" ::"r"(t0));  // Multiply filter(0,0) by the whole 1st Row
      asm volatile("vmacc.vx v0, %0, v10" ::"r"(t1)); // Multiply filter(1,0) by the whole 2nd Row 
      asm volatile("vmacc.vx v0, %0, v12" ::"r"(t2)); // Multiply filter(2,0) by the whole 3rd Row
      ...
      // Fetch the middle column of the filter, and start calculating its contributions on the output rows. To do so, slide down the Image rows by one
      f_ = f + 1;
      ...
      // Prepare for the next iteration: point to next image and output rows
      i += N + F - 1;
      o_ += N;
    }
  }
