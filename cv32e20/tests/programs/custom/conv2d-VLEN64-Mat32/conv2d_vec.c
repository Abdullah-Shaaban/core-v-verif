...
extern int8_t image[] __attribute__((aligned(4)));    // [ (R+2*floor(F/2)) * (C+2*floor(F/2)) ] = [ (R+1) * (C+1) ]
extern int8_t filter[] __attribute__((aligned(4)));   // [ F*F ]
extern int8_t output[] __attribute__((aligned(4)));   // [ R*C ]
extern int8_t golden_o[] __attribute__((aligned(4))); // [ R*C ]
...
int main(){
  ...
  conv2d_3x3(output, image, filter, R, C);
  ...
  //     // printf("Error at index %d: Expected %X, Got %X\n", indx, golden_o[indx], output[indx]);
  //     error +=1;
  //   }
  // }
  // // Check last 100 elements
  // for (uint32_t indx = R * C - 100; indx < R * C; indx++) {
  //   if (output[indx] != golden_o[indx]) {
  //     printf("Error at index %d: Expected %X, Got %X\n", indx, golden_o[indx], output[indx]);
  //   }
  // }
