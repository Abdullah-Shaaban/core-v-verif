...
void matmul_1xVL(int8_t *c, const int8_t *a, const int8_t *b, const unsigned int M, const unsigned int N, const unsigned int P) {
  ...
  asm volatile("vsetvli %0, %1, e8, m1, ta, ma" : "=r"(block_size_p) : "r"(P));
  ...
  while (p < P) {
    ...
    asm volatile("vsetvli %[gvl], %[vl], e8, m1, ta, ma" : [gvl] "=r"(gvl) : [vl] "r"(p_));
    ...
    for (unsigned int m = 0; m < M; m += 1) {
        ...
        for (int n=0; n < N; n++) {
            ...
            // Multiply
            asm volatile("vmul.vx v20, v20, %0" ::"r"(t0));
            // Accumulate
            asm volatile("vadd.vv v0, v0, v20" ::);
