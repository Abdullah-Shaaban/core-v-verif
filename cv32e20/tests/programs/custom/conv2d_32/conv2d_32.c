...
extern int32_t i[] __attribute__((aligned(4))); // [ (R+2*floor(F/2)) * (C+2*floor(F/2)) ]
extern int32_t f[] __attribute__((aligned(4)));        // [ F*F ]
extern int32_t o[] __attribute__((aligned(4)));        // [ R*C ]
extern int32_t golden_o[] __attribute__((aligned(4))); // [ R*C ]

void print_matrix(int32_t const *matrix, uint32_t num_rows, uint32_t num_columns) {
  printf("0x%8X\n", (uint32_t)matrix);
  for (uint32_t i = 0; i < num_rows; ++i) {
    for (uint32_t j = 0; j < num_columns; ++j) {
      printf("%8X ", (uint32_t)matrix[i * num_columns + j]);
    }
    printf("\n");
  }
}
...
int main(){
  ...
  int error = 0;
  for (uint32_t indx = 0; indx < R * C; indx++) {
    if (o[indx] != golden_o[indx]) {
      printf("Error at index %d: %d != %d\n", indx, o[indx], golden_o[indx]);
      error += 1;
    }
  }
  if (error != 0) {
