...
void conv2d_3x3(int *output, int *image, int *f) {
...
int main() {
  ...
  // Copy 8bit data into 32bit data
  int image_32[(M+2) * (N+2)];
  int filter_32[F*F];
  int golden_o_32[M*N];
  int output_32[M*N];
  for (int i = 0; i <(M+2) * (N+2); i++) {
    image_32[i] = (unsigned int)image[i];
  }
  for (int i = 0; i < F*F; i++) {
    filter_32[i] = (unsigned int)filter[i];
  }
  for (int i = 0; i < M*N; i++) {
  ...
  conv2d_3x3(output_32, image_32, filter_32);
  ...
  //   //   error = error + 1;
  //   // }
  //   if (output_32[indx] != golden_o_32[indx]) {
  //     printf("Error at index %d: %d != %d\n", indx, output_32[indx], golden_o_32[indx]);
  //     error = error + 1;
  //   }
  // }
  // if (error != 0) {
